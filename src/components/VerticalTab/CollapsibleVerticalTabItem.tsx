...
const ArrowIcon = ({ is_open }: { is_open: boolean }) => (
    <svg
        className={clsx("vertical-tab__arrow", {
...
export const CollapsibleVerticalTabItem = ({
    ...
    const selectedSubItemSelected = item?.subItems?.find(
        (subItem) => subItem.id === selectedTab,
    );
    const [open, setOpen] = useState(selectedSubItemSelected ? true : false);

    const onClickHandler = () => {
        const shouldCollapse = !selectedSubItemSelected;
        if (shouldCollapse) setOpen(!open);
    };
    return (
        <div key={item.id} className={clsx("collapsible-vertical-tab")}>
            <div
                className={clsx("collapsible-vertical-tab__header", {
                    "collapsible-vertical-tab__header--open":
                        selectedSubItemSelected,
                })}
                onClick={() => onClickHandler()}
            >
                <span className={clsx("vertical-tab__icon", iconClassName)}>
                    {" "}
                    {item?.icon}
                </span>
                <Text size="sm" className="vertical-tab__label">
                    {item.id}
                </Text>
                    ...
                    {item?.subItems?.map((subItem) => {
                        return (
                            <VerticalTabItem
                                ...
                                key={subItem.id}
                                tab={subItem}
                                onClick={() => onSelectItemHandler(subItem.id)}
                            />
                        );
                    })}
                </div>
